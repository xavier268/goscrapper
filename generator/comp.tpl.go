...
type TplData struct {
	...
	RunState       string // Name of the state constant pointing to the start State.
	...
	PoolSize       int    // PagePool size. 0 means no pool.
}
...
func (c *Compiler) setTplData() (err error) {
	...
	c.TplData.RunState = StateName(c.conf.Run)
	c.TplData.Headless = c.conf.Headless
...
// Generate a go file from the template(s) file with the same name.
func (c *Compiler) generateTpl(baseNames ...string) error {
